package Day40_060225.net.board.db;
...
public class BoardDAO {
	...
	// 글목록보기.
	//page 한 페이지에 limit 몇개의 글을 보여줄지
	//select문은 from절 조건절에 모두 사용될 수 있다.
	public List getBoardList(int page, int limit) {
		String board_list_sql = "select * from " + "(select rownum rnum, BOARD_NUM,BOARD_ID, BOARD_SUBJECT,"
				...
				+ "(select * from memberboard order by" + "BOARD_RE_REF desc, BOARD_RE_SEQ asc))"
				+ "where rnum>=? and rnum<=?";
		List list = new ArrayList();

		int startrow = (page - 1) * 10 + 1;// 읽기 시작할row번호.
	...
	// 글쓴이인지 확인.
	public boolean isBoardWrite(int num, String id) {
